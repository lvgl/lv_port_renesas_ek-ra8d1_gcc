...
static StaticTask_t LVGL_thread_memory;
                ...
                #else
static uint8_t LVGL_thread_stack[32768] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.LVGL_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
const dmac_extended_cfg_t g_transfer2_extend = { .offset = 0, .src_buffer_size =
		0,
...
const transfer_instance_t g_transfer2 = { .p_ctrl = &g_transfer2_ctrl, .p_cfg =
		&g_transfer2_cfg, .p_api = &g_transfer_on_dmac };
...
const ospi_b_xspi_command_set_t g_ospi0_high_speed_command_set =
		{ .protocol = SPI_FLASH_PROTOCOL_8D_8D_8D, .command_bytes =
				OSPI_B_COMMAND_BYTES_2, .read_command = 0xEEEE,
				.page_program_command = 0x1212, .write_enable_command = 0x0606,
				.status_command = 0x0505, .read_dummy_cycles = 10,
...
static const ospi_b_extended_cfg_t g_ospi0_extended_cfg = { .channel =
		(ospi_b_device_number_t) 1, .data_latch_delay_clocks = 0x00,
		...
		.p_autocalibration_preamble_pattern_addr = (uint8_t*) 0x90000000,
            ...
            #endif
		.read_dummy_cycles = 0, .program_dummy_cycles = 0, /* Unused by OSPI Flash */
		.status_dummy_cycles = 0, };
const spi_flash_cfg_t g_ospi0_cfg = { .spi_protocol =
		SPI_FLASH_PROTOCOL_1S_1S_1S, .read_mode = SPI_FLASH_READ_MODE_STANDARD, /* Unused by OSPI Flash */
.address_bytes = SPI_FLASH_ADDRESS_BYTES_4, .dummy_clocks =
		SPI_FLASH_DUMMY_CLOCKS_DEFAULT, /* Unused by OSPI Flash */
...
.page_size_bytes = 64, .write_status_bit = 0, .write_enable_bit = 1,
		.page_program_command = 0x12, .write_enable_command = 0x06,
		.status_command = 0x05, .read_command = 0x13,
...
#endif
		.erase_command_list_length = sizeof(g_ospi0_erase_command_list)
				/ sizeof(g_ospi0_erase_command_list[0]), .p_erase_command_list =
				&g_ospi0_erase_command_list[0], .p_extend =
				&g_ospi0_extended_cfg, };
...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts =
		(uint32_t) 0x3a980, .duty_cycle_counts = 0x1d4c0, .source_div =
		(timer_source_div_t) 0, .channel = 0, .p_callback = timer_tick_callback,
/** If NULL then do not add & */
    ...
    .p_context           = NULL,
		...
		.p_context = &NULL,
#endif
		.p_extend = &g_timer0_extend, .cycle_end_ipl = (8),
...
const iic_master_extended_cfg_t g_i2c_master1_extend =
		{ .timeout_mode = IIC_MASTER_TIMEOUT_MODE_LONG, .timeout_scl_low =
				...
				/* Actual calculated bitrate: 398936. Actual calculated duty cycle: 50%. */.clock_settings.brl_value =
						31, .clock_settings.brh_value = 31,
				.clock_settings.cks_value = 2, };
const i2c_master_cfg_t g_i2c_master1_cfg = { .channel = 1, .rate =
		I2C_MASTER_RATE_FAST, .slave = 0x5D, .addr_mode =
...
#undef RA_NOT_DEFINED
		.p_callback = g_i2c_master1_cb, .p_context = NULL,
...
void LVGL_thread_create(void) {
	...
	LVGL_thread = xTaskCreateStatic(
                    ...
                    #endif
			LVGL_thread_func, (const char*) "LVGL Thread", 32768 / 4, // In words, not bytes
