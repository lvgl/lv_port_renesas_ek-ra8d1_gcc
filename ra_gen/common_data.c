...
const external_irq_cfg_t g_external_irq3_cfg = { .channel = 3, .trigger =
		...
		EXTERNAL_IRQ_CLOCK_SOURCE_DIV_64, .p_callback = touch_irq_callback,
...
#define MIPI_PHY_TINIT     (143999)
#define MIPI_PHY_TCLKPREP  (17)
#define MIPI_PHY_THSPREP   (11)
#define MIPI_PHY_TCLKTRAIL (14)
#define MIPI_PHY_TCLKPOST  (39)
#define MIPI_PHY_TCLKPRE   (3)
#define MIPI_PHY_TCLKZERO  (55)
#define MIPI_PHY_THSEXIT   (23)
#define MIPI_PHY_THSTRAIL  (16)
#define MIPI_PHY_THSZERO   (38)
#define MIPI_PHY_TLPEXIT   (14)

/* MIPI PHY Structures */
const mipi_phy_timing_t g_mipi_phy0_timing = { .t_init = 0x3FFFF
		& (uint32_t)MIPI_PHY_TINIT, .t_clk_prep = (uint8_t)MIPI_PHY_TCLKPREP,
		.t_hs_prep = (uint8_t)MIPI_PHY_THSPREP, .dphytim4_b.t_clk_trail =
				(uint32_t)MIPI_PHY_TCLKTRAIL, .dphytim4_b.t_clk_post =
				(uint32_t)MIPI_PHY_TCLKPOST, .dphytim4_b.t_clk_pre =
				(uint32_t)MIPI_PHY_TCLKPRE, .dphytim4_b.t_clk_zero =
				(uint32_t)MIPI_PHY_TCLKZERO, .dphytim5_b.t_hs_exit =
				(uint32_t)MIPI_PHY_THSEXIT, .dphytim5_b.t_hs_trail =
				(uint32_t)MIPI_PHY_THSTRAIL, .dphytim5_b.t_hs_zero =
...
const mipi_phy_cfg_t g_mipi_phy0_cfg =
		{
				.pll_settings =
						/* Calculated MIPI PHY PLL frequency: 1000000000 Hz (error 0.00%) = 20000000 Hz / 1 * 50.00 */{
...
const mipi_dsi_extended_cfg_t g_mipi_dsi0_extended_cfg = { .dsi_seq0.ipl = (12),
		.dsi_seq0.irq = VECTOR_NUMBER_MIPIDSI_SEQ0,

		.dsi_seq1.ipl = (12), .dsi_seq1.irq = VECTOR_NUMBER_MIPIDSI_SEQ1,

		.dsi_vin1.ipl = (12), .dsi_vin1.irq = VECTOR_NUMBER_MIPIDSI_VIN1,

		.dsi_rcv.ipl = (12), .dsi_rcv.irq = VECTOR_NUMBER_MIPIDSI_RCV,

		.dsi_ferr.ipl = (12), .dsi_ferr.irq = VECTOR_NUMBER_MIPIDSI_FERR,

		.dsi_ppi.ipl = (12), .dsi_ppi.irq = VECTOR_NUMBER_MIPIDSI_PPI,
		...
		.dsi_plie = 0x0, .dsi_vmie = R_DSILINK_VMIER_VBUFUDF_Msk
				| R_DSILINK_VMIER_VBUFOVF_Msk | 0x0, .dsi_sqch0ie =
				R_DSILINK_SQCH0IER_AACTFIN_Msk | R_DSILINK_SQCH0IER_ADESFIN_Msk
						| R_DSILINK_SQCH0IER_TXIBERR_Msk
						| R_DSILINK_SQCH0IER_RXFERR_Msk
						| R_DSILINK_SQCH0IER_RXFAIL_Msk
						| R_DSILINK_SQCH0IER_RXPFAIL_Msk
						| R_DSILINK_SQCH0IER_RXCORERR_Msk
						| R_DSILINK_SQCH0IER_RXAKE_Msk | 0x0, .dsi_sqch1ie =
				R_DSILINK_SQCH1IER_AACTFIN_Msk | R_DSILINK_SQCH1IER_ADESFIN_Msk
						| R_DSILINK_SQCH1IER_SIZEERR_Msk
						| R_DSILINK_SQCH1IER_TXIBERR_Msk
						| R_DSILINK_SQCH1IER_RXFERR_Msk
						| R_DSILINK_SQCH1IER_RXFAIL_Msk
						| R_DSILINK_SQCH1IER_RXPFAIL_Msk
						| R_DSILINK_SQCH1IER_RXCORERR_Msk
...
const mipi_dsi_cfg_t g_mipi_dsi0_cfg =
		...
		.sync_pulse = (0), .data_type = MIPI_DSI_VIDEO_DATA_24RGB_PIXEL_STREAM,
				...
				.vertical_active_lines = 854, .vertical_sync_lines = 3,
				.vertical_back_porch = (20 - 3), .vertical_front_porch = (894
						- 854 - 20 - 3), .vertical_sync_polarity =
						(DISPLAY_SIGNAL_POLARITY_LOACTIVE
								!= DISPLAY_SIGNAL_POLARITY_HIACTIVE),

				.horizontal_active_lines = 480, .horizontal_sync_lines = 2,
				.horizontal_back_porch = (5 - 2), .horizontal_front_porch = (559
						- 480 - 5 - 2), .horizontal_sync_polarity =
						(DISPLAY_SIGNAL_POLARITY_LOACTIVE
								!= DISPLAY_SIGNAL_POLARITY_HIACTIVE),

				.video_mode_delay = 78 /* This value was calculated by FSP. An override is available but not recommended for most users */,
				...
				.num_lanes = 2, .ulps_wakeup_period = 97, .continuous_clock =
						(1),

				.hs_tx_timeout = 0, .lp_rx_timeout = 0, .turnaround_timeout = 0,
				.bta_timeout = 0, .lprw_timeout = (0
						<< R_DSILINK_PRESPTOLPSETR_LPRTO_Pos) | 0,
				.hsrw_timeout = (0 << R_DSILINK_PRESPTOHSSETR_HSRTO_Pos) | 0,

				.max_return_packet_size = 1, .ecc_enable = (1),
				.crc_check_mask = (mipi_dsi_vc_t)(0x0), .scramble_enable = (0),
				.tearing_detect = (0), .eotp_enable = (1),

				.p_extend = &g_mipi_dsi0_extended_cfg, .p_callback =
						mipi_dsi0_callback, .p_context = NULL, };
...
const glcdc_extended_cfg_t g_display0_extend_cfg = { .tcon_hsync =
		...
		GLCDC_CLK_SRC_INTERNAL, .clock_div_ratio = GLCDC_PANEL_CLK_DIVISOR_8,
...
const display_cfg_t g_display0_cfg =
		...
		{ .coordinate = { .x = 0, .y = 0 }, .bg_color = { .byte = { .a = 255,
				...
				.output =
						{ .htiming = { .total_cyc = 559, .display_cyc = 480,
								.back_porch = 5, .sync_width = 2,
								.sync_polarity =
										DISPLAY_SIGNAL_POLARITY_LOACTIVE },
								.vtiming =
										{ .total_cyc = 894, .display_cyc = 854,
												.back_porch = 20, .sync_width =
														3,
												.sync_polarity =
														DISPLAY_SIGNAL_POLARITY_LOACTIVE },
				...
				.p_callback = glcdc_callback, .p_context = NULL,
